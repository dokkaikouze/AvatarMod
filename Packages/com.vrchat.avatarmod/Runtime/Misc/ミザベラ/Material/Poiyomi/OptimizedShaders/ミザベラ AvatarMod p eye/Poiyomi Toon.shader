Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/fa5fb4a1cc4c83a42ae6d5415a6ae433"
{
	Properties
	{
		...
		[Helpbox(1, 2, 1)] _Decal0PosHelp ("To apply Positioning using Raycast, edit this Material directly from your Mesh in the Scene. Click for video tutorial.--{onClick:{type:URL,data:https://www.poiyomi.com/color-and-normals/decals#positioning}}", Int) = 0
		...
		[HideInInspector][ThryToggleUI(true)]_DecalColorAdjustEnabled ("Color Adjust", Float) = 0
		...
		[Space(7)]
		[HideInInspector] s_start_Decal0ALColorOverrideS ("Color Override--{reference_property:_AudioLinkDecal0ColorOverrideEnabled, persistent_expand:true, default_expand:false}", Float) = 0
		...
		[ThryWideEnum(Smooth, 0, Flashing, 1)]_EmissiveBlink_Mode ("Mode", Int) = 0
		...
		[Space(4)]
		[HideInInspector] s_start_EmissionAL0Section ("Audio Link ♫--{condition_showS:_EnableAudioLink==1, reference_property:_EmissionAL0Enabled, persistent:expand:true, default_expand:false}", Float) = 0
 ...
 #define OPTIMIZER_ENABLED 
			...
			float _AudioLinkDecal0ColorOverrideEnabled;
			float4 _AudioLinkDecal0ColorOverride;
			float _AudioLinkDecal0ColorOverrideThemeIndex;
			float _AudioLinkDecal0ColorOverrideBand;
			float4 _AudioLinkDecal0ColorOverrideRamp;
			...
			float _DecalColorAdjustEnabled;
			float _DecalSaturation;
			float _DecalChromatize;
			...
			float _EmissiveBlink_Mode;
			float _EmissiveBlink_Duty;
			...
			void applyMainColor(inout PoiFragData poiFragData, inout PoiMesh poiMesh, inout PoiCam poiCam, inout PoiMods poiMods, inout float4 mainTexture)
			{
				...
				if (2.0)
				{
					...
					#if defined(PROP_ALPHAMASK) || !defined(OPTIMIZER_ENABLED)
					alphaMaskTex = POI2D_SAMPLER_PAN(_AlphaMask, _MainTex, poiUV(poiMesh.uv[0.0], float4(8,8,0,0)), float4(0,0,0,0).xy);
					#endif
					float alphaMask = poiSampleMask(alphaMaskTex, float4(1.0, 0.0, 0.0, 0.0), 2.0, 0.0, float4(0,1,0,1), 1.0, 1.0, _AlphaMaskValue);
					switch ((int)2.0)
					{
			...
			float2 decalUV(float uvNumber, float2 position, half rotation, half rotationSpeed, half2 scale, float4 scaleOffset, float depth, in float symmetryMode, in float mirroredUVMode, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float theta = radians(rotation + POI_TIME.z * rotationSpeed);
				float cs = cos(theta);
				float sn = sin(theta);
			...
			inline float3 decalHueShift(float enabled, float3 color, float shift, float shiftSpeed, float colorSpace, float selectOrShift)
			{
				if (enabled)
				{
					color = hueShift(color, shift + POI_TIME.x * shiftSpeed, colorSpace, selectOrShift);
				}
				return color;
			}
			inline float3 decalColorAdjust(float enabled, float3 color, float saturation, float chromatize, float brightness, float gamma)
			{
				if (!enabled)
				{
					return color;
				...
				color = lerp(color, dot(color, float3(0.3, 0.59, 0.11)), -saturation);
				if (chromatize != 0)
				{
					#if !defined(UNITY_COLORSPACE_GAMMA)
					float3 chromaLinear = color;
					#else
					float3 chromaLinear = OpenLitSRGBToLinear(color);
					#endif
					chromaLinear = PoiApplyChromatize(chromaLinear, chromatize);
					#if !defined(UNITY_COLORSPACE_GAMMA)
					color = saturate(chromaLinear);
					#else
					color = saturate(OpenLitLinearToSRGB(chromaLinear));
					#endif
				}
				color = saturate(color * (brightness + 1));
				return color;
			}
			...
			struct PoiDecal
			{
				...
				float m_DecalColorAdjustEnabled;
				float m_DecalSaturation;
				float m_DecalChromatize;
				float m_DecalBrightness;
				float m_DecalGamma;
				float m_DecalDepth;
				...
				float m_AudioLinkDecalColorOverrideEnabled;
				float4 m_AudioLinkDecalColorOverride;
				float m_AudioLinkDecalColorOverrideThemeIndex;
				float m_AudioLinkDecalColorOverrideBand;
				float4 m_AudioLinkDecalColorOverrideRamp;
				#endif
				float4 decalColor;
				float2 decalScale;
				float decalRotation;
				...
				void SampleDecalChannelSeparation(sampler2D decalTexture, in PoiMods poiMods, in PoiLight poiLight, in PoiMesh poiMesh, in PoiCam poiCam)
				{
					...
					float4 sample0 = tex2D(decalTexture, poiUV(uvSample0, m_DecalTexture_ST) + m_DecalTexturePan * POI_TIME.x, dduvSample0.xy, dduvSample0.zw) * float4(poiThemeColor(poiMods, m_DecalColor.rgb, m_DecalColorThemeIndex), m_DecalColor.a);
					float4 sample1 = tex2D(decalTexture, poiUV(uvSample1, m_DecalTexture_ST) + m_DecalTexturePan * POI_TIME.x, dduvSample1.xy, dduvSample1.zw) * float4(poiThemeColor(poiMods, m_DecalColor.rgb, m_DecalColorThemeIndex), m_DecalColor.a);
				...
				void Apply(inout float alphaOverride, inout float decalAlpha, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
				{
					...
					if (poiMods.audioLinkAvailable && m_AudioLinkDecalColorOverrideEnabled)
					{
						float alColorOverrideIntensity = poiMods.audioLink[m_AudioLinkDecalColorOverrideBand];
						float alColorOverrideBlend = evalRamp4(alColorOverrideIntensity, m_AudioLinkDecalColorOverrideRamp);
						if(alColorOverrideBlend > 0)
						{
							float3 alOverrideColor = m_AudioLinkDecalColorOverride.rgb;
							if(m_AudioLinkDecalColorOverrideThemeIndex > 0)
							{
								alOverrideColor = poiThemeColor(poiMods, alOverrideColor, m_AudioLinkDecalColorOverrideThemeIndex);
							}
							decalColor.rgb = lerp(decalColor.rgb, alOverrideColor, alColorOverrideBlend);
						}
					}
					#endif
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
			{
				...
				Decal0.m_DecalColorAdjustEnabled = 0.0;
				Decal0.m_DecalSaturation = 0.0;
				Decal0.m_DecalChromatize = 0.0;
				Decal0.m_DecalBrightness = 0.0;
				Decal0.m_DecalGamma = 1.0;
				...
				Decal0.ApplyAspectRatio(float4(0.0004882813,0.0004882813,2048,2048).zw);
				#endif
				#if defined(POI_AUDIOLINK)
				...
				Decal0.m_AudioLinkDecalColorOverrideEnabled = 0.0;
				Decal0.m_AudioLinkDecalColorOverride = float4(1,1,1,1);
				Decal0.m_AudioLinkDecalColorOverrideThemeIndex = 0.0;
				Decal0.m_AudioLinkDecalColorOverrideBand = 0.0;
				Decal0.m_AudioLinkDecalColorOverrideRamp = float4(0,1,0,1);
			...
			)
			{
				...
				poiLight.finalLightAdd += max(0.0, finalSpec * 0.5);
				if (0.0 > 0.0)
				{
			...
			float calculateBlinkingEmission(in float blinkMin, in float blinkMax, in float blinkVelocity, in float offset, in float mode, in float duty)
			{
				if (mode == 1)
				{
					float phase = frac((POI_TIME.y + offset) * blinkVelocity / (2.0 * UNITY_PI));
					float on = step(phase, duty);
					return lerp(blinkMin, blinkMax, on);
				}
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				float glowInTheDarkMultiplier0 = calculateGlowInTheDark(0.35, 0.8, 1.0, 0.0, _EnableGITDEmission, 0.0, poiLight);
				#ifdef POI_GRABPASS
				...
				emissionColor0 = emissionTex.rgb * lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, 0.0);
				emissionAlpha = emissionTex.a;
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, 0.0);
				#endif
				...
				if (0.0)
				{
					emissionStrength0 *= calculateBlinkingEmission(0.0, 1.0, 4.0, 0.0, 0.0, 0.5);
				}
				if(0.0)
				{
			...
			float4 frag(VertexOut i, bool facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 emissionBaseReplace2 = 0;//<ms_no_postfix>
				float3 emissionBaseReplace3 = 0;//<ms_no_postfix>
				#endif
				#endif
				#ifdef _EMISSION
				emissionBaseReplace += applyEmission(poiFragData, poiMesh, poiLight, poiCam, poiMods);
				#ifdef TRIPLANAR_PROJECTION
				emissionBaseReplace2 += applyEmission(poiFragData2, poiMesh2, poiLight, poiCam, poiMods);
				emissionBaseReplace3 += applyEmission(poiFragData3, poiMesh3, poiLight, poiCam, poiMods);
				#endif
				#endif
				#if defined(_EMISSION)|| defined(POI_EMISSION_1)|| defined(POI_EMISSION_2)|| defined(POI_EMISSION_3)
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, saturate(emissionBaseReplace), poiMax(emissionBaseReplace));
				#ifdef TRIPLANAR_PROJECTION
				poiFragData2.baseColor.rgb = lerp(poiFragData2.baseColor.rgb, saturate(emissionBaseReplace2), poiMax(emissionBaseReplace2));
				poiFragData3.baseColor.rgb = lerp(poiFragData3.baseColor.rgb, saturate(emissionBaseReplace3), poiMax(emissionBaseReplace3));
				#endif
				#endif
				...
				{
					float LTCGIsmoothness = 0.965;
					float LTCGImetalness = 0.374;
					...
					acc.specular *= poiThemeColor(poiMods, float4(0.7077566,0.7077566,0.7077566,1).rgb, 0) * 1.0;
